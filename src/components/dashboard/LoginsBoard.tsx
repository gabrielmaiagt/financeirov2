...
const LoginsBoard = () => {
  ...
  const loginsQuery = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'logins'), orderBy('title')) : null),
    [firestore]
  );
  ...
  const handleSaveLogin = (loginData: Omit<Login, 'id'>) => {
    ...
    if (editingLogin) {
      const loginRef = doc(firestore, 'logins', editingLogin.id);
  ...
  const handleDeleteLogin = (id: string) => {
    if (!firestore) return;
    deleteDocumentNonBlocking(doc(firestore, 'logins', id));
  };
