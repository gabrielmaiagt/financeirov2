...
const NotesBoard = () => {
  ...
  const notesQuery = useMemoFirebase(
    () => (firestore ? query(collection(firestore, 'anotacoes'), orderBy('createdAt', 'desc')) : null),
    [firestore]
  );
  ...
  const handleSaveNote = (noteData: Omit<Anotacao, 'id'>) => {
    if (!firestore) return;
    if (editingNote) {
      const noteRef = doc(firestore, 'anotacoes', editingNote.id);
      updateDoc(noteRef, noteData);
    } else {
      const notesRef = collection(firestore, 'anotacoes');
  ...
  const handleDeleteNote = (id: string) => {
    if (!firestore) return;
    deleteDocumentNonBlocking(doc(firestore, 'anotacoes', id));
  };
